...
- **Placard meets newsroom.** GetUp's chunky purple identity as full-bleed banner bars with
  knocked-out caps, sitting inside newspaper furniture — flat rules, no rounded corners,
  screen-printed halftone art built from CSS gradients (no image files, no external requests).
...
- **Two themes.** Light and dark are both designed, not inverted. Story art keeps the same ink
  in both — a printed image doesn't change colour when the reader flips their theme.

### Swapping in real brand assets

Every colour is a custom property at the top of `css/getacross.css`. Replacing GetUp's real
palette means editing the `:root` block and its two dark-mode counterparts — nothing else
references a raw hex. The type stack is two variables in the same block (`--serif`, `--grot`);
